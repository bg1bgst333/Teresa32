...
int CTitleScene::InitGameObjects(){
	...
	m_pSelectBox->Create(160, 300, 320, 120, m_pMainWnd->m_hWnd, IDB_SELECTBOX1, 24, _T("ＭＳ ゴシック"), 20);	// m_pSelectBox->Createでセレクトボックスオブジェクトを作成.
...
int CTitleScene::DrawGameObjects(){
	...
	if (m_pTitle != NULL){	// m_pTitleがNULLでない時.
		...
		m_pTitle->DrawText(160, 60, 320, 240, _T("Teresa"), RGB(0xff, 0xff, 0xff));	// m_pTitle->DrawTextでタイトルテキストの描画.
	}
